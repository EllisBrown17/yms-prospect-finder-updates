...
BASE='https://raw.githubusercontent.com/EllisBrown17/yms-prospect-finder-updates/10e6f10a7ecbfe8048fc301d92527d80b7f3d1ed/releases/5.0.6/'
...
new='''  function findNavButton(label){
    ...
    return els.find(e=>{const sig=[e.textContent,e.getAttribute('onclick'),e.getAttribute('href'),e.getAttribute('data-tab'),e.getAttribute('data-view'),e.id].filter(Boolean).join(' ').toLowerCase();return sig.includes(q)}) || els.find(e=>{const sig=[e.textContent,e.getAttribute('onclick'),e.getAttribute('data-tab'),e.id].filter(Boolean).join(' ').toLowerCase();return /prospect|product|discover|dashboard|settings/.test(sig)});
  }'''
if old not in js:
    raise SystemExit('The CRM navigation patch pattern was not found in the verified 5.0.6 JS. Nothing was changed.')
...
# Verify the clean server is actually serving the CRM-patched page before creating the launcher.
try:
    check_url=f'http://127.0.0.1:{port}/?verify={int(time.time())}'
    with urllib.request.urlopen(check_url,timeout=8) as r:
        served=r.read().decode('utf-8',errors='ignore')
    if 'YMS CRM CANONICAL 5.0.6 START' not in served or 'v506CrmNav' not in served:
        raise RuntimeError('The running server did not serve the CRM-patched page.')
    print('Verified: the running YMS page contains the CRM UI patch.')
except Exception as ex:
    raise SystemExit('Clean YMS started, but CRM page verification failed: '+str(ex))
